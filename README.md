...
First, add the `Houssam\Permission\Traits\HasRoles` trait to your `User` model(s):
...
The scope can accept a string, a `\Houssam\Permission\Models\Role` object, a `\Houssam\Permission\Models\Permission` object or an `\Illuminate\Support\Collection` object.
...
The `assignRole`, `hasRole`, `hasAnyRole`, `hasAllRoles`  and `removeRole` functions can accept a
 string, a `\Houssam\Permission\Models\Role` object or an `\Illuminate\Support\Collection` object.
...
string or a `Houssam\Permission\Models\Permission` object.
...
All these responses are collections of `Houssam\Permission\Models\Permission` objects.
...
- Your `Role` model needs to extend the `Houssam\Permission\Models\Role` model
- Your `Permission` model needs to extend the `Houssam\Permission\Models\Permission` model

If you need to extend or replace the existing `Role` or `Permission` models you just need to
keep the following things in mind:

- Your `Role` model needs to implement the `Houssam\Permission\Contracts\Role` contract
- Your `Permission` model needs to implement the `Houssam\Permission\Contracts\Permission` contract
